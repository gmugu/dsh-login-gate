 ...
 *
 * v1.7.0: LAN settings access. The dsh web client treats every non-loopback
 * page as untrusted and keeps the Host settings document process-local, so
 * over `http://<lan-ip>:3080` the Models page fails with "settings are
 * unavailable in this browser", General renders nothing, and plugin config
 * forms cannot persist. This half now taps the served SPA index
 * (`webServer.tapIndex`) to seed `__DSH_TRANSPORT__.ownsHost = true` — the
 * documented desktop trust carrier — before any client module boots, so the
 * password-authenticated page is treated as loopback-trusted and every
 * settings surface works over LAN. `trustLanSettings: false` restores the
 * upstream conservative behavior. The `/api` Host/Origin 403 fence stays
 * untouched: reaching the page by DNS hostname still requires dsh's
 * `--trusted-host`.
 */
...
/**
 * Head script seeded into the served SPA index when `trustLanSettings` is on.
 * `__DSH_TRANSPORT__` is dsh's documented page-level trust carrier: its
 * `ownsHost: true` flag is how the desktop shell marks its page as loopback-
 * trusted, and `dsh-client-connection` reads it before any other client module
 * applies. The merge keeps fields a static page may already carry
 * (`streamBaseUrl`), so the only behavioral change is
 * `connection.isLoopback === true` — which re-enables the settings document
 * (Models directory, General page, plugin config forms) on LAN pages this
 * gate has password-protected. Inline in <head>, it runs during parse, before
 * every module script, so the flag is deterministic rather than racy.
 */
const LAN_TRUST_SNIPPET =
	'<script>try{var t=globalThis.__DSH_TRANSPORT__;if(t==null){t={};globalThis.__DSH_TRANSPORT__=t}t.ownsHost=true}catch(e){}</script>';
...
const Config = z.object({
	...
	/**
	 * Trust the password-gated page on non-loopback origins with the Host
	 * settings document. The dsh web client marks every page whose hostname is
	 * not localhost/127.x/[::1] as untrusted and refuses to load the settings
	 * document there ("settings are unavailable in this browser" on the Models
	 * page; General and plugin config forms disabled). With this on, the served
	 * SPA index seeds `__DSH_TRANSPORT__.ownsHost = true` (the desktop trust
	 * carrier) so those surfaces work over LAN. Takes effect on the next index
	 * load after this plugin (re)mounts. The /api 403 fence is never touched.
	 */
	trustLanSettings: z.boolean().default(true),
});
...
function apply(ctx, config) {
	...
	/**
	 * LAN settings trust: seed `__DSH_TRANSPORT__.ownsHost = true` into the
	 * served SPA index so the dsh web client treats this password-gated page as
	 * loopback-trusted and loads the Host settings document over LAN. The tap
	 * is the webserver's official index transform; `renderIndex` applies it to
	 * the frontend dist's index on every index request. Disposed with the
	 * plugin fiber, so disable/remount restores the plain index.
	 */
	if (config.trustLanSettings !== false) {
		const ws = ctx.webServer;
		if (typeof ws?.tapIndex !== "function") {
			console.log("[login-gate] webserver has no tapIndex; LAN settings patch skipped");
		} else {
			const removeTap = ws.tapIndex((html) => {
				const open = /<head(?:\s[^>]*)?>/i.exec(html);
				if (open === null) return `${LAN_TRUST_SNIPPET}${html}`;
				const at = open.index + open[0].length;
				return `${html.slice(0, at)}${LAN_TRUST_SNIPPET}${html.slice(at)}`;
			});
			ctx.effect(() => removeTap, "login-gate: lan trust index tap");
			console.log("[login-gate] LAN pages trusted for the settings document (trustLanSettings)");
		}
	}
