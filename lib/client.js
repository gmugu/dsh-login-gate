...
.lgc-msg-err{color:#ff9aa4}`;
		...
		// dsh 0.1.7: the client settingsScope service is gone, so the TTL
		// editor talks to the gate's own auth-gated /__auth/settings bridge
		// (the host half reads and writes through the official settings
		// service — same persistence as the Plugins config form). Only
		// `slots` is declared (provided by ui-renderer); a hard dependency
		// on a missing service would park this half forever.
		var inject = ["slots"];

		function fetchSettings() {
			return fetch("/__auth/settings", { credentials: "same-origin" }).then(function (res) {
		 ...
		 * General / Models / Plugins / Agent presets): session TTL (through
		 * the gate's /__auth/settings bridge), password change/reset, and
		 * logout — all backed by auth-gated routes of the host half.
		 */
		function LoginGateSection() {
			var msgState = react.useState(null);
			var msg = msgState[0];
			var setMsg = msgState[1];

			var loadState = react.useState({ status: "loading" });
			var load = loadState[0];
			var setLoad = loadState[1];
			react.useEffect(function () {
				var alive = true;
				fetchSettings().then(function (r) {
					if (alive) setLoad(r);
				});
				return function () { alive = false; };
			}, []);

			var ready = load.ok === true;
			var ttlValue = ready && typeof load.ttlHours === "number" ? String(load.ttlHours) : "";

			var ttlState = react.useState(typeof ttlValue === "string" && ttlValue !== "" ? ttlValue : "");
			var ttl = ttlState[0];
			var setTtl = ttlState[1];
			react.useEffect(function () {
				setTtl(typeof ttlValue === "string" && ttlValue !== "" ? ttlValue : "");
			}, [ttlValue]);
			...
			function saveTtl() {
				...
				setMsg(null);
				var fields = { ttlHours: String(Math.floor(n)) };
				if (typeof load.revision === "number") fields.revision = String(load.revision);
				saveSettings(fields).then(function (r) {
					if (r.ok) {
						setLoad(r);
						setMsg({ kind: "ok", text: "会话时长已保存，即时生效。" });
					} else {
						setMsg({ kind: "err", text: r.error || "保存失败，请重试。" });
					}
				});
			}
			...
			return h("div", { className: "lgc-section" },
				...
				) : h("div", { className: "lgc-field" },
					h("label", { className: "lgc-label" }, "会话时长（小时）"),
					h("p", { className: "lgc-hint" }, load.status === "loading" ? "正在读取设置…" : (load.error || "设置服务不可用，暂时无法在线修改。"))
				),
				h("div", { className: "lgc-field" },
